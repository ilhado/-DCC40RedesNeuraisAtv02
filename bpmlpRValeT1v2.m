% DCC40 - Atividade 02
% Reconhecimento de padrões via RNA - Questão 1
...
D = baseOriginal (:,7)';
% arquivoSaida = fopen('C:\Users\oficial\Documents\DCCMAPI\Atv02\saidaT45.mat','w');
% arquivoSaidaTreinos = fopen('C:\Users\oficial\Documents\DCCMAPI\Atv02\saidaTreinos45.mat','w');
...
% fprintf(arquivoSaida,'\n\nTreinamento nº - %d\n', treinamento);
% fprintf(arquivoSaida,'Taxa de aprendizagem - %f\n',eta);
...
% fprintf(arquivoSaidaTreinos,'Taxa de aprendizagem - %f\n',eta);
...
while Eav > Emax 
    ...
    Eav = mse(E);
    OcorrenciasDeEavs(end+1)=Eav;

end

% fprintf(arquivoSaida,'Valores Mínimos de Eav na iteracao nº %d\n', iteracao);
% formatSpec = '%.4f \t';
% fprintf(arquivoSaida,formatSpec,OcorrenciasDeEavs);
% fprintf(arquivoSaida,'\n\n\n');

ValoresDeReferencia(end+1) = D(1,iteracao+1); 
ValoresDeSaida(end+1) = round(mse(Y));
iteracao = iteracao+1;
end
...
axis([-0.5 amplitude 0 5]);
plot(ValoresDeSaida, "r");
plot(ValoresDeReferencia, "k");
title("Resultados do aprendizado via BPMLP");
legend('Saída da Rede', 'Saída de Referência');
